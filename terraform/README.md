...
The RDS master password is generated by AWS itself
(`manage_master_user_password` on `modules/rds`) - Terraform never sees it as
plaintext - and rotates automatically every `password_rotation_days` (default
30) via Secrets Manager's built-in RDS rotation, no custom Lambda involved.
The one secret Terraform *can't* generate is the Slack webhook URL
...
Either way (the Slack URL above, or an RDS password rotation firing), ECS
only resolves `secrets` values at task start, not continuously - ECS won't
notice on its own, so an app container needs to actually restart
(redeploy, or `aws ecs update-service --force-new-deployment`) to pick up a
changed value.
...
## Security & DR (Phase 10)

- **IAM**: every ECS execution/task role is scoped per-service, not shared -
  see the inline comments in `modules/iam` for where AWS simply doesn't
  support tighter (resource-level) scoping and why. Phase 7 (CI/CD, not yet
  merged) adds a similarly-scoped `pulse-github-deploy` OIDC role for
  deploys - see that phase's `bootstrap/github_oidc.tf` once it lands.
- **Security groups**: `modules/security_groups` - each service's SG only
  allows inbound from exactly what needs to reach it (api from the ALB only,
  checker from scheduler only, the database from api/scheduler/checker only).
- **HTTPS**: off by default (no domain/ACM certificate assumed) - set
  `certificate_arn` on `module.alb` in an environment once one exists, and
  the ALB adds an HTTPS listener and redirects HTTP to it instead of serving
  plaintext.
- **Secrets rotation**: see above.
- **Disaster recovery**: RTO/RPO targets per failure scenario, and which
  ones are actually proven (not just claimed) via a `scripts/chaos/` drill -
